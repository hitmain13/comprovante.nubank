import { createComprovante } from './create-comprovante'
import * as api from './api'

describe('createComprovante', () => {
  beforeEach(() => {
  ...
  it('cria comprovante com sucesso', async () => {
    ...
    const result = await createComprovante({ payload: { foo: 'bar' }, latitude: 1, longitude: 2 })
  ...
  it('cria comprovante sem latitude/longitude', async () => {
    ...
    const result = await createComprovante({ payload: { foo: 1 } })
  ...
  it('propaga erro do apiFetch', async () => {
    ...
    await expect(createComprovante({ payload: { foo: 'bar' } })).rejects.toThrow('fail')
  })

  it('envia payload correto', async () => {
    const spy = jest.spyOn(api, 'apiFetch').mockResolvedValueOnce({ hash: 'ok' })
    await createComprovante({ payload: { a: 1 }, latitude: 2, longitude: 3 })
  ...
  it('usa URL padrão se env não definido', async () => {
    ...
    await createComprovante({ payload: { a: 1 } })
    expect(api.apiFetch).toHaveBeenCalledWith(
  ...
  it('envia body como string', async () => {
    const spy = jest.spyOn(api, 'apiFetch').mockResolvedValueOnce({ hash: 'ok' })
    await createComprovante({ payload: { test: true } })
